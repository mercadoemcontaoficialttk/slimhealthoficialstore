...
export function PixPaymentModal({
  ...
  useEffect(() => {
    if (!isOpen) { setTimeLeft(900); return; }
  ...
  // Build QR image source: prefer base64, fallback to generated URL from qrCode text
  const qrImageSrc = qrCodeBase64
    ? (qrCodeBase64.startsWith('data:') ? qrCodeBase64 : qrCodeBase64.startsWith('http') ? qrCodeBase64 : `data:image/png;base64,${qrCodeBase64}`)
    : qrCode
        ...
        <div className="p-6">
          ...
          {!isLoading && !error && (
            <>
              ...
              {qrCode && (
                <p className="text-xs text-gray-400 text-center mt-4 break-all line-clamp-2 px-2">{qrCode}</p>
              )}
            </>
          )}
        </div>
